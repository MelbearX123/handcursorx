...
ref_path = os.path.join(input_folder, video_files[0]) #Grab the path of the first video file
ref_cap = cv2.VideoCapture(ref_path)                  #Capture the first video

target_fps = ref_cap.get(cv2.CAP_PROP_FPS)            #Take the fps of the first video and use it as reference
target_w = int(ref_cap.get(cv2.CAP_PROP_FRAME_WIDTH)) #Take the width of the first video and use it as reference
target_h = int(ref_cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) #Take the height of the first video and use it as reference
...
out = cv2.VideoWriter(output_video, fourcc, target_fps, (target_w, target_h)) #Write the new video
